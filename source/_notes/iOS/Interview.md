...
- 不能添加成员变量？
  ...
  - 为什么不加：对象的内存布局在编译期就已经定型了，由于 category 在运行时才被合并进原生类 ，这时候直接添加成员变量会破坏类原有的布局。（注意和后面的 Non-fragile ivars 做区分，父类的变更会体现在其结构体中，所以可以直接知道布局对不上，方便做处理，其实按道理可以做，但是不合算，不管是性能还是代码复杂度）
