...
这时候就需要有一种检测方案，可以实时检测对象之间是否发生了循环引用，来辅助开发者及时修正代码存在的内存泄漏问题。要想检测出循环引用，最直观的方式是递归地获取对象强引用的其他对象，并判断检测对象是否被其路径上的对象强引用了，也就是在有向图中去找环。，明确在已知强引用链的情况下如何查找循环引用之后，接下来需要解决的是如何获取强引用链，也就是获取对象的强引用，尤其是最易产生循环引用的 block。
...
首先解决捕获变量排序的问题，根据 [clang 针对这部分的排序代码](https://github.com/llvm-mirror/clang/blob/e870496ea61feb01aa0eb4dc599be0ddf2d03878/lib/CodeGen/CGBlocks.cpp#L366-L384)，我们可以知道，在对齐字节数 (alignment) 不相等时，捕获的实体按照 alignment 降序排序 (C struct 比较特殊，即使占用空间比指针变量大，也排在对象指针后面，这块逻辑还为找到对应的代码去验证)，否则按照以下类型进行排序 :
...
由于结构体的布局顺序在声明时就已经确定了，无法像 block 构造捕获区域一样，按照变量类型、修饰符进行排序调整，所以如果结构体中有类型为对象指针的字段，需要一些额外信息来计算这些对象指针字段的偏移量。
